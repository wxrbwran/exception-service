--docker run --name postgres -e POSTGRES_PASSWORD=123456 -p 5432:5432 -v /root/docker/pg:/var/lib/postgresql/data -d postgres:latest

create table if not exists s_exception.t_user
(
    id            serial not null
...
create table if not exists s_exception.t_organization
(
    id           serial  not null
...
create unique index if not exists t_organization_id_uindex
    on s_exception.t_organization (id);

create table if not exists s_exception.t_project
(
    id                   serial not null
...
create unique index if not exists t_project_id_uindex
    on s_exception.t_project (id);

create table if not exists s_exception.t_notification_setting
(
    id         serial not null
...
create unique index if not exists t_notification_setting_id_uindex
    on s_exception.t_notification_setting (id);

create table if not exists s_exception.t_invite
(
    id           serial not null
...
create unique index if not exists t_invite_id_uindex
    on s_exception.t_invite (id);

create table if not exists s_exception.t_notification_rule
(
    id         serial not null
...
create unique index if not exists t_notification_rule_id_uindex
    on s_exception.t_notification_rule (id);

create table if not exists s_exception.t_source_map
(
    id          serial not null
...
create unique index if not exists t_source_map_id_uindex
    on s_exception.t_source_map (id);

create table if not exists s_exception.t_issue
(
    id           serial not null
...
comment on column s_exception.t_issue.events is '所对应的 events (doc:_id)';
